"""
Originally written by Bellet et al. Modified by Kellen Vu.
"""
...
def main(argv):
    ...
    ##### TRAINING MODE #####
        ...
        try:
            ...
            print(
                'At least one input argument is missing: UnEye.py -x <xfile> -y <yfile> -l <saccfile> -f '
                '<samplingfreq>')
            sys.exit(2)
        ...
        if kappa < 0.7:
            print(
                "Bad performance can be due to an insufficient size of the training set, high noise in the data or "
                "incorrect labels. Check your data and contact us for support.")
        ...
        if type(X) == str:
            ...
            if X.endswith('mat'):
                ...
                for c in range(classes):
                    np.savetxt(parent + "/Labels_prob_" + "class" + str(c) + ".csv", Probability[:, c, :],
                               delimiter=",")
