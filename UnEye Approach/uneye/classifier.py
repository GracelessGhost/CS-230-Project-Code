...
class DNN():
    ...
    weights_name: str, filename of weights to save or load, will automatically be stored and load in local folder
    'training', default: 'weights'
    ...
    def train(self, X, Y, Labels, seed=1):
        ...
        Labels: array-like, shape: (n_samples, n_timepoints), event labels: fixation=0, saccade=1 (other classes
        optional)
        ...
        # __ samples of training data used for validation
        ...
        while epoch <= self.max_iter:
            ...
            if len(Loss_val) > 3:
                if Loss_val[-1] < float(
                        np.mean(Loss_val[-4:-1])):  # validation performance better than average over last 3
                    getting_worse = 0
        ...
        if save_weights:
            ...
            print(
                "Model parameters could not be saved due to early overfitting. Try to reduce learning rate or "
    ...
    def predict(self, X, Y):
        ...
        Parameters
        ----------
        X,Y: array-like, shape: {(n_timepoints),(n_samples, n_timepoints)}, horizontal and vertical eye positions in
        degree
                
        Output
        ------
        Pred: array-like, shape: {(classes, n_timepoints),(n_samples, n_timepoints)}, class prediction,
        values in range [0 classes-1], fixation=0, saccades=1
        
        Prob: array-like, shape: {(classes, n_timepoints),(n_samples, classes, n_timepoints)}, class probabilits (
    ...
    def test(self, X, Y, Labels):
        ...
        Labels: array-like, shape: {(n_timepoints),(n_samples, n_timepoints)}, class labels in range [0 classes-1],
        ...
        if classes == 2:
            ...
            true_pos, false_pos, false_neg, on_distance, off_distance = accuracy(Pred.astype(float),
            ...
            true_pos, false_pos, false_neg, on_distance, off_distance = accuracy((Pred == 1).astype(float),
                                                                                 (Labels == 1).astype(float))
    ...
    def crossvalidate(self, X, Y, Labels, X_val, Y_val, Labels_val, Labels_test=None, K=10):
        ...
        X_val,Y_val: array-like, shape: {(n_timepoints),(n_samples, n_timepoints)}, additional horizontal and
        vertical eye positions in degree for validation
        
        Labels_val: array-like, shape: {(n_timepoints),(n_samples, n_timepoints)}, additional class labels in range [
        0 classes-1], fixation=0, saccades=1 for validation
        
        Labels_test: array-like, shape: {(n_timepoints),(n_samples, n_timepoints)}, if test Labels different from
        training labels (for training with missing labels only), optional
        ...
        for i in range(K):
            ...
            true_pos, false_pos, false_neg, on_distance, off_distance = accuracy((Prediction == 1).astype(float),
                                                                                 Ltest[:, 1, :])
            f1 = (2 * true_pos) / (2 * true_pos + false_neg + false_pos)
            print('F1:', np.round(f1, 3))
            F1[i] = f1
