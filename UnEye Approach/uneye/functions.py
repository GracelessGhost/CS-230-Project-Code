...
def binary_prediction(output, samp_freq, p=0.5, min_sacc_dist=1, min_sacc_dur=1):
    ...
    Output
    ------
    S_predic: array-like, shape={(n_timepoints), (n_samples,n_timepoints)}, fixation(=0) and saccade(=1) prediction
    for each time_bin in the network output
...
def accuracy(predicted, truth):
    '''
    Computes accuracy of prediction compared to groundtruth (in terms of true and false positive and false negative
    detections)
    ...
    for i in range(batchsize):
        ...
        if len(start_t) != 0:  # if there is at least one saccade
            ...
            for j in range(len(start_t)):
                ...
                content_of_pred = pred_copy[i, int(start_t[j]):int(end_t[j])]  # content of prediction during true
                # saccade duration
                ...
                if content_of_pred != []:
                    if np.mean(content_of_pred) == 0:
                        ...
                        if content_of_pred[0] == 1:  # the matching predicted saccade start earlier than the ground truth
                            ...
                            ind_before_true_start = pred_start_ind[
                                pred_start_ind < start_t[j]]  # predicted start before true start

                            if len(ind_before_true_start) == 0:
                                start = 0;  # set predicted start to first bin because it is before the start of the
                                # trace
                            else:
                            ...
                            if pred_copy[i, -1] == 1:
                                # if the prediction ends with a saccade, set the last bin of the differential to -1
                                # to add an end
                                diff_pred_copy[-1] = -1

                                # get the predicted end bin:
                            pred_end_ind = np.argwhere(diff_pred_copy == -1) + 1  # get all saccade end bins in trace
                            ind_after_start = pred_end_ind[
                                pred_end_ind > start]  # only consider end bins that occur after start
                            end = ind_after_start[0]  # use first end bin after predicted saccade start as saccade end

                            pred_copy[i, cluster_belonging(pred_copy[i, :].astype(bool),
                                                           start_t[j])] = 0  # delete matching pairs
                            truth_copy[i, cluster_belonging(truth_copy[i, :], start_t[
                                j])] = 0  # to not consider them for the next iteration
                            true_pos += 1
                            ...
                            start = ind_from_true_start[
                                        0] + 1  # set predicted start index to first found after true start
...
def EM_saccade_detection(X, Y, lambda_param=6, min_sacc_dist=5, min_sacc_dur=10, sampfreq=1000):
    # Addapted from Engbert, Ralf, and Konstantin Mergenthaler. "Microsaccades are triggered by low retinal image
    # slip." Proceedings of the National Academy of Sciences 103.18 (2006): 7192-7197.
    ...
    for i in np.arange(0, np.shape(X)[0]):
        ...
        Sacc_out[i, :] = (np.divide(Vx[i, :], (lambda_param * np.sqrt(Eta_x[i]))) ** 2 + np.divide(Vy[i, :], (
                    lambda_param * np.sqrt(Eta_y[i]))) ** 2) > 1
        ...
        for j in np.arange(1, np.max(labels) + 1):
            ...
            if np.sum(labels == j) < int(
                    min_sacc_dur * (sampfreq / 1000)):  # remove saccades smaller than min_sacc_dur (ms)
